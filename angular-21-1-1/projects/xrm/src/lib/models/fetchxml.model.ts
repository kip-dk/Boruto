...
export class FetchEntity {
  constructor(name: string, entityPrototype: Entity, attributes?: string[], condition?: Condition, linkedentities?: Link[], parent?: FetchEntity) {
    this.name = name;
  ...
  link(prototype: Entity, property: string, condition?: Condition, attributes?: string[]): FetchEntity {
    ...
    var next = new Link(this.name, property, "inner", property, res);
  ...
  innerjoin(prototype: Entity, alias: string, from: string, property: string, condition?: Condition, attributes?: string[]): FetchEntity {
    ...
    var next = new Link(from, alias, "inner", property, res);
  ...
  private resolveAlias(links: Link[], result: string[]): void {
    if (links != null && links.length > 0) {
      links.forEach(l => {
        ...
        if (l.entity.linkedentities != null) {
...
export class Link {
  constructor(from: string, alias: string, type: string, to: string, entity: FetchEntity) {
    this.from = from;
    this.alias = alias;
    this.type = type;
    this.to = to;
...
export class Fetchxml {
  ...
  constructor(prototype: Entity, condition: Condition) {
    this.root = new FetchEntity(prototype._logicalName, prototype, [], condition);
  ...
  sort(attribname: string, descending?: boolean): void {
    ...
    let nextsort = new Fetchsort(attribname, descending ?? false);
