...
namespace Boruto.Deployment.Services
{
    internal class SdkMessageProcessingStepService : ServiceAPI.ISdkMessageProcessingStepService
    {
        ...
        internal static string ImageName(int pre1post2)
        {
        ...
        {
            var name = ImageName(pre1post2);
            var query = Entities.SdkMessageProcessingStepImage.EntityLogicalName.ToQueryExpression();
            query.Criteria.Equal(Entities.SdkMessageProcessingStepImage.Fields.SdkMessageProcessingStepId, crmStep.SdkMessageProcessingStepId.Value);
            query.Criteria.Equal(Entities.SdkMessageProcessingStepImage.Fields.Name, name);

            var existingImages = this.orgService.RetrieveMultiple(query).Entities.Select(r => new Entities.SdkMessageProcessingStepImage(r)).ToArray();


            if (imgDef == null || (existingImages != null && existingImages.Length > 1))
            {
                if (existingImages != null && existingImages.Length > 0)
                {
                    foreach (var ee in existingImages)
                    {
                        this.orgService.Delete(ee.LogicalName, ee.Id);
            ...
            var existingImage = existingImages != null ? existingImages.SingleOrDefault() : null;
        ...
        private void CreateImage(SdkMessageProcessingStep crmStep, int pre1post2, int stage, bool async, string message, Models.Image imgDef)
        {
            var image = new SdkMessageProcessingStepImage
            {
                ...
                Name = ImageName(pre1post2),
                EntityAlias = ImageName(pre1post2),
            ...
            messageService.Inform($"Created image {ImageName(pre1post2)} on step {crmStep.Name.Split('.').Last()}");
        }
