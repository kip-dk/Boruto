...
## DISCLAIMER
We are still in process of stabilize the concept and improving documentation. We will make an expicit RELEASED note when the concept is ready for adoption.
...
### 1.4.3.19 2025-10-12 Generic extension method to sort hiracy data with children on top of the list

### 1.4.3.18 2025-10-05 Single Plugin listenting to multi Organization Request in same stage was wrongly called for any of the request. Fix has been made so only method having organiztaionrequest input that match current message.

### 1.0.3.11 2025-09-15 Lock around static Dictionary operations to avoid index-out-of-bound on multi threads accessing same dic.

### 1.0.3.10 2025-09-14 Deployment tool fixed, if [Sort] was changed for a step, already deployed, it did not reflect in the Rank of the step

### 1.0.3.9 2025-09-13 Impl. safe way to get access to current IServiceContext from a static context, extension method on BasePlugin

### 1.0.3.7 2025-09-13 Impl. safe way to get access to current plugin context from a static context, extension method on BasePlugin

### 1.0.3.6 2025-09-08 Fixed rare and strange null ref. on dictionary in method resolver

### 1.0.3.5 2025-09-04 IsSame added DateTime compare based wil fall back to string compare on format ("yyyyMMddHHmmmss") unless d1 and d2 are same in a date compare.

### 1.0.3.4 2025-08-19 Fix entitytype resolver

### 1.0.3.3 2025-08-19 Admin flag still not repsected on inject of custom provider created service directly on plugin method. Fixed.

### 1.0.3.2 2025-08-19 Admin flag not always respected when custom service provider should create object service instance

### 1.0.3.1 2025-08-18 Extension method IsSame is adjusted so string compare consider empty string same as null. Version bump du to change in IsSame api behavior
